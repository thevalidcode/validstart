...
async function scaffoldFrontendJS(options) {
    ...
    if (framework !== "React") {
        console.log(chalk_1.default.red(`❌ Only React is supported for JavaScript frontend at the moment.`));
        return;
    }
    console.log(chalk_1.default.cyan(`\n🚧 Creating React + Vite project: ${chalk_1.default.bold(projectName)}`));
    ...
    if (selectedTools.includes("shadcn/ui")) {
        ...
        console.log(chalk_1.default.yellow("⚠️ You will need to manually initialize shadcn/ui afterwards."));
    }
    if (toolDeps.length > 0) {
        console.log(chalk_1.default.blue(`📦 Installing additional tools: ${toolDeps.join(", ")}`));
